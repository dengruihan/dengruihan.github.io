...
- **章节时钟保持**（engine.js `fieldClock(t, sY)`，结果写进 `holds.ft`，blend / cameraAt / fog 共用）：用户反馈"卡片还没走完波浪就变形了"、"timeline 部分不要有镜头移动"、"第一张卡片上移前正方体就开始消散"——`blend(t)` 原本在每个 t∈[k,k+1] 全程插值，章节演出全程踩在变形上。改为分段重映射：t<3.85 钉在 3（journey 卡片全程波浪完整、相机停 journey 机位）；t∈[3.85,4.0] 压缩完成 wave→deck；**t∈[4,5) 钉在 4 直到 projects scroll zone 刷完（sY≥zoneEnd），deck→blog 变形压缩进 zoneEnd→focus5 一段**（马赛克在全部三次翻页中保持成形，翻完才散）；t≥5 恒等。`chapterWeight()` 及 story.update 里所有 w 门控都跟随 holds.ft 自动同步；journey 卡片本身锚在相机空间、projects 卡片锚世界坐标但相机同样被钉住，均不受影响。注意卡片演出窗口 `show`、projects 门控 `w4Gate`、deck 翻页 `holds.deck` 用的是**原始 t / 原始滚动**，不要改成重映射后的值。
...
- **deck 面板延迟启动**：用户反馈方块还没归位面板就开始移动——deck 原本由 `scrub[4]`（整个 scroll zone 的滚动进度）驱动，到达章节中心时已消耗约一半滚动。engine 在 holds 里新增 `deck = clamp01((sY - focus4)/(zoneEnd - focus4))`，story 改用 `holds.deck` 算 `_deckCi`——t=4（方块就位）前面板全部静止，之后一片片上移，t≈4.53 最后一张到位（代价：每张面板的滚动行程约为原来的一半）。随后用户明确目标效果：**成对垂直翻页**——左侧马赛克面板 + 右侧项目卡片作为一对，滚动时一起向上飞出屏幕、露出下一对（中间一度改成"双相位"（卡片先行、马赛克后动），非用户所愿，已回退）。实现（`_deckCi` 单索引同步驱动 + `deckPanelPose` 出场 liftY 3.8→9 直飞出屏、缩放缩减 0.85→0.25 保持完整、出场卡片 frontness 改为全程 0.8 仅在末段淡出）：马赛克与卡片共享同一 liftY，同步飞走。
验证：fly 探针——card0 y 从 -1.50 升到 7.50 全程 op 0.80（飞行中不淡出），card1 同步显露；desk_* 桌面模式截图（headless 需 `add_init_script` 覆盖 `navigator.deviceMemory=8` 才能退出 lite）确认左马赛克/右卡片布局与成对飞出。若嫌阅读时间仍短，下一步是调高 `setProjectsHeight` 的 perCard 加长滚动房间（会等比拉长 journey 演出窗口，需同步确认）。
